...
export function resolveRoot(roots, absHostPath) {
  ...
  for (const root of roots) {
    ...
    if (samePath(absHostPath, rootPath)) {
      const candidate = { root, relativePath: '', rootPathLength: rootPath.length };
      if (!bestMatch || candidate.rootPathLength > bestMatch.rootPathLength) {
        bestMatch = candidate;
      }
      continue;
    }
    ...
    if (absHostPath.toLowerCase().startsWith(prefix.toLowerCase())) {
      ...
      const candidate = {
        root,
        relativePath: relative.split(path.win32.sep).join('/'),
  ...
  return { root: bestMatch.root, relativePath: bestMatch.relativePath };
}
