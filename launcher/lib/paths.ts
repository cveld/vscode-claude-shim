// Pure path-validation/translation logic for the launcher API/backend (see
// docs/plan-launcher.md, decisions 5-7). No filesystem or Docker access here — this
...
export type ResolvedPathType = 'workspace' | 'folder';
...
export function toAbsoluteHostPath(raw: string): string {
...
export function resolveRoot(roots: Root[], absHostPath: string): ResolvedRoot | null {
...
export function toContainerPath(root: Root, relativePath: string): string {
  if (!relativePath) return root.containerPath;
...
export function toHostPath(root: Root, relativePath: string): string {
  if (!relativePath) return trimTrailingSep(path.win32.resolve(root.hostPath));
...
export function classifyByExtension(absHostPath: string): ResolvedPathType {
...
export function resolveHostPath(roots: Root[], raw: string): ResolvedHostPath | null {
