// Docker-touching half of the launcher API/backend (docs/plan-launcher.md, decision 4): talks
// to Docker Desktop via its Windows named pipe through dockerode, never by shelling out to
// `docker` with string-concatenated arguments. No in-memory instance registry — listInstances
// always re-queries `docker ps` by label, so a launcher restart needs no recovery step.

import Docker from 'dockerode';
import crypto from 'node:crypto';
import { claudeHomeMounts } from './claudeHome.ts';
...
export async function createInstance(resolved) {
  ...
  const container = await docker.createContainer({
    ...
    Cmd: ['--auth', 'none', '--bind-addr', '0.0.0.0:8080', cmdTarget],
    Env: [],
